...
def main():
    ...
    try:
        for iteration in range(times):
            ...
            perform_action("touch", "4.png", "「前日のデータ」をタップする", logger, delay=10)

            custom_print('「前日」ボタンを4回タップする', update_step=True, color=Fore.LIGHTCYAN_EX)
            for times_zen in range(4):
                perform_action("touch", "5.png", f"「前日」をタップする({times_zen+1}回目)", logger, delay=3)

            custom_print('「翌日」ボタンを4回タップする', update_step=True, color=Fore.LIGHTCYAN_EX)
            for times_yoku in range(4):
                perform_action("touch","6.png", f"「翌日」をタップする({times_yoku+1}回目)", logger, delay=3)

            perform_action("wait", "7.png","検出中：「翌日」ボタンの状態を確認中", logger,use_rgb=True)

            perform_action("touch", "8.png", "「自分の記録」をタップする", logger, delay=3)

            perform_action("touch", "9.png", "「X」をタップする", logger, delay=3)
            
            perform_action("touch", "10.png", "「カテゴリーで見る」をタップする", logger, delay=3)

            perform_action("touch", "9.png", "「X」をタップする", logger, delay=3)
            
            perform_action("touch", "11.png", "「廃棄予算設定」をタップする", logger, delay=3)

            perform_action("touch", "12.png", "「原価」をタップする", logger, delay=3)
            
            perform_action("wait", "13.png", "検出中：「原価」の状態を確認中", logger, use_rgb=True)
            
            perform_action("touch", "14.png", "「AC」をタップする", logger, delay=3)

            perform_action("touch", "15.png", "「設定する」ボタンの状態を確認", logger, delay=3)

            perform_action("touch", "16.png", "「1」ボタンをタップする", logger, delay=3)
        ...
        format_json_line_by_line(platform_config['logfile'])  # ここでログファイルをフォーマットする
